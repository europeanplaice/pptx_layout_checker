from utils import (
    ...
    namespaces,
    check_func,
    func_to_whole_process,
    function_for_all_font,
)
import tempfile
...
def test_font_shape_default_font():
    with tempfile.TemporaryDirectory() as dir:
        fontlist = func_to_whole_process(
            "font_sample/shape_with_default_font.pptx", dir
        )
...
def test_font_abadi_default_no():
    with tempfile.TemporaryDirectory() as dir:
        fontlist = func_to_whole_process(
            "font_sample/font_abadi_and_default_and_shape_with_no_text.pptx", dir
        )
    assert set(fontlist) == set(
        [
            (None, "楕円 1"),
            ("Abadi", "テキスト ボックス 3"),
            ("default", "テキスト ボックス 2"),
